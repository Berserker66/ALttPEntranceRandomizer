...
def distribute_items_restrictive(world, gftower_trash=False, fill_locations=None):
    ...
    for player in range(1, world.players + 1):
        if not gftower_trash or not world.ganonstower_vanilla[player] or \
                world.logic[player] in {'owglitches', "nologic"} or world.doorShuffle[player] == 'crossed':
    ...
    if not balanceable_players:
        ...
        while True:
            ...
            if checked_locations:
                ...
                balancing_players = [player for player, reachables in reachable_locations_count.items() if reachables < threshold and player in balanceable_players]
