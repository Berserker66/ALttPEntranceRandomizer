...
class Sprite(object):
    ...
    def default_link_sprite():
        return Sprite.get_sprite_from_name('Link')
...
def patch_rom(world, rom, player, team, enemized, is_mystery=False):
                ...
                if isinstance(exit.addresses, tuple):
                    ...
                    if world.shuffle[player] not in {'insanity', 'insanity_legacy', 'madness_legacy'} and \
                            exit.name in {'Eastern Palace Exit', 'Tower of Hera Exit', 'Thieves Town Exit',
                                          'Ice Palace Exit', 'Misery Mire Exit',
    ...
    if world.fix_skullwoods_exit[player] and world.shuffle[player] == 'vanilla':
        ...
        rom.write_int16(0x15DB5 + world.force_fix[player]['sw'].exit_offset, 0x00F8)
    if world.force_fix[player]['pod']:
        rom.write_int16(0x15DB5 + world.force_fix[player]['pod'].exit_offset, 0x0640)
    if world.force_fix[player]['tr']:
        rom.write_int16(0x15DB5 + world.force_fix[player]['tr'].exit_offset, 0x0134)
    if world.force_fix[player]['gt']:
        rom.write_int16(0x15DB5 + world.force_fix[player]['gt'].exit_offset, 0x00A4)
    ...
    if prevent_rain:
        ...
        for idx, portal in enumerate(portals):
            ...
            rom.write_int16(0x13f0f0+x, room_idx)
    ...
    if world.mode[player] == 'standard':
        if uncle_location.item and uncle_location.item.player == player:
            if uncle_location.item.name in {'Bow', 'Progressive Bow'}:
                ...
                bow_max, bow_small, uncle_refill = 70, 10, 1
        ...
        if world.doorShuffle[player] == 'crossed':
            ...
            rom.write_bytes(0x18018E, [3, 1])
            rom.write_byte(0x180196, 1)
    ...
    rom.name = bytearray(f'BD{__version__.replace(".", "")[0:3]}_{team + 1}_{player}_{world.seed:09}\0', 'utf8')[:21]
...
def write_pots_to_rom(rom, pot_contents):
    ...
    for i in range(0x140):
        if i in pot_contents:
            ...
            if len(pots) > 0:
                rom.write_int16(pot_item_room_table_lookup + 2*i, n)
                rom.write_bytes(n, list(itertools.chain(*((pot.x,pot.y,pot.item) for pot in pots))))
                n += 3*len(pots) + 2
