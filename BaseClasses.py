...
class Region(object):
    ...
    def can_fill(self, item: Item):
        ...
        sewer_hack = self.world.mode[item.player] == 'standard' and item.name in ['Small Key (Hyrule Castle)', 'Big Key (Hyrule Castle)']
        ...
        with open(filename, 'w', encoding="utf-8-sig") as outfile:
            ...
            for player in range(1, self.world.players + 1):
                ...
                if self.metadata['door_shuffle'][player] != "vanilla":
                    ...
                    outfile.write('Door Shuffle Seed                %s\n' % self.metadata['dr_seeds'][player])
            ...
            if self.unreachables:
                ...
                # items: Item names
                outfile.write('\n\nUnreachable Items:\n\n')
                outfile.write('\n'.join(['%s: %s (%s)' % (self.world.fish.translate("meta", "items", unreachable.item.name),
                                                     self.world.fish.translate("meta", "locations", unreachable.name),
                                                          self.world.get_player_names(unreachable.item.player))
